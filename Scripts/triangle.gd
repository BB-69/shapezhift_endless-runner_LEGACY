...
func spin(delta):
	if velocity.length() > 0:
		var spin_direction = sign(velocity.x)
		var max_speed = 6000.0
		var speed_factor = clamp(abs(velocity.length()), 0, max_speed) / max_speed
		var target_rotation = rad_to_deg(spin_direction * speed_factor)
		aspr.rotation += target_rotation * rotation_speed * delta
